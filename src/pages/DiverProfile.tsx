...
function StatisticsTab({ diverId }: { diverId: string }) {
  ...
  const grouped = useMemo(() => {
    const map: Partial<Record<BoardType, CompStatEntry[]>> = {};
        ...
        <div className="flex gap-5">
          ...
          <div className="w-56 shrink-0 border border-slate-700 rounded-xl overflow-hidden max-h-72 overflow-y-auto">
            {diveOptions.length === 0 ? (
              ...
              BOARD_ORDER.filter((b) => grouped[b]?.length).map((board) => (
                <div key={board}>
                  <div className="bg-slate-800 px-3 py-1.5 text-xs font-semibold text-slate-400 uppercase tracking-wide sticky top-0">
                    {board}
                  </div>
                  {grouped[board]!.map((entry) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 text-xs flex items-center gap-2 transition-colors cursor-pointer hover:bg-slate-800/60 ${
