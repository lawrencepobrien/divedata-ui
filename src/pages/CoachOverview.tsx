...
function CoachOverview({ coachName }: Props): JSX.Element {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      <div className="mb-10">
        <h1 className="text-3xl font-bold mb-1">{coachName}</h1>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {roster.map((diver) => (
            <div
              key={diver.diver_id}
              onClick={() => navigate(`/roster/${diver.diver_id}`)}
              className="bg-slate-900 border border-slate-800 rounded-xl p-5 transition duration-150 hover:border-cyan-500 cursor-pointer"
            >
              ...
              {diver.email && <p className="text-slate-500 text-sm">{diver.email}</p>}
