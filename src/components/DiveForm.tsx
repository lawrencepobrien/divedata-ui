...
interface Props {
  initialValues?: Partial<DiveFormValues>;
  submitLabel: string;
  submittingLabel: string;
  isSubmitting: boolean;
  errorMessage?: string | null;
  onSubmit: (payload: CreateDiveRequest) => void;
  /** Extra fields (e.g. a video picker) rendered inside the same card, above the submit row. */
...
export default function DiveForm({
  ...
  const [diveCode, setDiveCode] = useState(initialValues?.diveCode ?? '');
  const [board, setBoard] = useState<BoardType>(initialValues?.board ?? '3m');
  const [date, setDate] = useState(initialValues?.date ?? (() => new Date().toISOString().slice(0, 10))());
  const [totalScore, setTotalScore] = useState(initialValues?.totalScore ?? '');
  const [judgeScores, setJudgeScores] = useState<string[]>(initialValues?.judgeScores ?? []);
  ...
  const canSubmit = diveCode.trim().length > 0 && date.length > 0 && (hasTotalScore || hasJudgeScores);
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
              ...
              <div className="flex gap-2 flex-wrap mt-1">
                {annotateJudgeScores(validJudgeScores, (s) => s).map(({ item: score, dropped, dropSide }, i) => (
                  ...
                  >
                    ...
                    {dropped && (
                      <span className="text-[10px] font-sans text-slate-700 font-normal">dropped ({dropSide})</span>
                    )}
                  </div>
                ))}
              </div>
      ...
      <div className="flex items-center gap-3">
        ...
        {!canSubmit && diveCode.trim().length > 0 && !hasTotalScore && !hasJudgeScores && (
          <span className="text-sm text-slate-500">Enter a total score or at least one judge score.</span>
        )}
